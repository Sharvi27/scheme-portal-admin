...
export function Input({ label, value, onChange, type = 'text', placeholder, required, style = {} }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
      {label && <label style={{ fontSize: '0.78rem', fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label}{required && ' *'}</label>}
      <input
...
export function Spinner() {
  return <div style={{ width: 18, height: 18, border: '2.5px solid var(--border)', borderTopColor: 'var(--navy)', borderRadius: '50%', animation: 'spin 0.7s linear infinite', display: 'inline-block' }} />
}
...
export function Toast({ message, type = 'success', onClose }) {
  return (
    <div style={{
      ...
      <button onClick={onClose} style={{ background: 'none', border: 'none', color: 'white', marginLeft: 8, cursor: 'pointer', fontSize: '1rem' }}>×</button>
    </div>
  )
}
